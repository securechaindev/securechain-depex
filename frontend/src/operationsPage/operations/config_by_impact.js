...
const ConfigByImpactOperation = (props) => {
  ...
  const [impact, set_impact] = useState('')

  const [max_level_error, set_max_level_error] = useState('')
  const [impact_error, set_impact_error] = useState('')

  const on_button_config_by_impact_operation = () => {
    set_max_level_error('')
    set_impact_error('')
    ...
    if (impact < 0 && impact > 10) {
      set_impact_error('The impact must be between 0 and 10')
      return
    }
    ...
    fetch('http://localhost:8000/operation/config/config_by_impact', {
      ...
      body: JSON.stringify({ requirement_file_id, max_level, impact, package_manager, agregator })
    })
      .then((r) => r.json())
      .then((r) => {
  ...
  return (
    <div className='flex-col flex'>
      <p>Config By Impact Operation</p>
      <input
        value={_max_level}
        type='number'
        min='-1'
      ...
      <input
        ...
        type='button'
        onClick={on_button_config_by_impact_operation}
        value={'Apply'}
      />
    </div>
  )
