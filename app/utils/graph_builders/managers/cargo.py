...
from app.apis import get_cargo_package, get_cargo_versions
...
async def cargo_create_requirement_file(requirement_file_name: str, file: dict[str, Any], repository_id: str) -> None:
    ...
    await cargo_generate_packages(file.get("requirement", {}), new_req_file_id)
...
async def cargo_create_package(
    ...
    versions, repository_url, vendor = await get_cargo_versions(package_name)
    if versions:
        attributed_versions = [
...
async def cargo_extract_packages(
    ...
    requirement = await get_cargo_package(
        parent_package_name, version.get("name")
    )
    await cargo_generate_packages(requirement, version.get("id"), parent_package_name)


async def cargo_search_new_versions(package: dict[str, Any]) -> None:
    versions, _, _ = await get_cargo_versions(package.get("name"))
    count = await count_number_of_versions_by_package("CargoPackage", package.get("name"))
    if count < len(versions):
        new_attributed_versions: list[dict[str, Any]] = []
        actual_versions = await read_versions_names_by_package("CargoPackage", package.get("name"))
        ...
        await update_versions_serial_number("CargoPackage", package.get("name"), versions)
        for version in created_versions:
            await cargo_extract_packages(package.get("name"), version)
    await update_package_moment("CargoPackage", package.get("name"))
