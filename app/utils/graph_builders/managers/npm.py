...
async def npm_create_requirement_file(requirement_file_name: str, file: dict[str, Any], repository_id: str) -> None:
    ...
    await npm_generate_packages(file.get("requirement"), new_req_file_id)
...
async def npm_create_package(
    ...
    if versions:
        ...
        for requirement, created_version in zip(requirements, created_versions):
            await npm_generate_packages(requirement, created_version.get("id"), package_name)


async def npm_search_new_versions(package: dict[str, Any]) -> None:
    versions, requirements = await get_npm_versions(package.get("name"))
    count = await count_number_of_versions_by_package("NPMPackage", package.get("name"))
    if count < len(versions):
        new_attributed_versions: list[dict[str, Any]] = []
        new_requirements = []
        actual_versions = await read_versions_names_by_package("NPMPackage", package.get("name"))
        ...
        await update_versions_serial_number("NPMPackage", package.get("name"), versions)
        for version, requirement in zip(created_versions, new_requirements):
            await npm_generate_packages(requirement, version.get("id"), package.get("name"))
    await update_package_moment("NPMPackage", package.get("name"))
