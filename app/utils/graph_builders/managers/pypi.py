...
async def pypi_create_requirement_file(requirement_file_name: str, file: dict[str, Any], repository_id: str) -> None:
    ...
    await pypi_generate_packages(file.get("requirement"), new_req_file_id)
...
async def pypi_extract_packages(
    ...
    await pypi_generate_packages(requirement, version.get("id"), parent_package_name)


async def pypi_search_new_versions(package: dict[str, Any]) -> None:
    versions, _, _ = await get_pypi_versions(package.get("name"))
    count = await count_number_of_versions_by_package("PyPIPackage", package.get("name"))
    if count < len(versions):
        new_attributed_versions: list[dict[str, Any]] = []
        actual_versions = await read_versions_names_by_package("PyPIPackage", package.get("name"))
        ...
        await update_versions_serial_number("PyPIPackage", package.get("name"), versions)
        for version in created_versions:
            await pypi_extract_packages(package.get("name"), version)
    await update_package_moment("PyPIPackage", package.get("name"))
