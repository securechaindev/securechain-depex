...
class GraphToSMT(Transformation):
    ...
    def mean(impacts: list[ArithRef]) -> ArithRef:
        if impacts:
            ...
            return If(num_non_zero > 0., sum(impacts) / num_non_zero, 0.)
        return 0.
    ...
    def weighted_mean(impacts: list[ArithRef]) -> ArithRef:
        if impacts:
            ...
            return If(divisors > 0., sum([val**2 * 0.1 for val in impacts]) / divisors, 0.)
        return 0.