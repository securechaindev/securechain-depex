...
async def no_exist_package(
    ...
    if all_versions:
        ...
        no_existing_versions = await generate_versions(all_versions, new_package_name)

        for data in no_existing_versions:
            version, require_packages = data
            await generate_packages(version, require_packages)


async def generate_versions(all_versions: list[dict[str, Any]], package_name: str) -> list[dict[str, Any]]:
    no_existing_versions: list[dict[str, Any]] = []
    cpe_matches = await read_cpe_matches_by_package_name(package_name)

    counter = 0
    for version in all_versions:
        require_packages = version.pop('require_packages')
        ...
        no_existing_versions.append([new_version, require_packages])
