...
async def no_exist_package(
    ...
    if all_versions:
        ...
        versions = [
            await relate_cves(version, cpe_matches, "MVN", package_name, artifact_id)
...
async def generate_packages(parent_package_name: str, version: dict[str, Any]) -> None:
    require_packages = await requires_packages(
        parent_package_name, version["name"], "MVN"
    )
    if require_packages:
...
async def search_new_versions(package: dict[str, Any]) -> None:
    ...
    if counter < len(all_versions):
        cpe_matches = await read_cpe_matches_by_package_name(
