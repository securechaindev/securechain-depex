...
async def mvn_extract_graph(name: str, file: Any, repository_ids: dict[str, str]) -> None:
    ...
    for dependencie, constraints in file['dependencies'].items():
        package = await read_package_by_name(dependencie, 'MVN')
        if package:
            if package['moment'] < datetime.now() - timedelta(days=10):
                await search_new_versions(package)
            await relate_package(dependencie, constraints, new_req_file_id, 'MVN')
        else:
...
async def search_new_versions(package: dict[str, Any]) -> None:
    ...
    if counter < len(all_versions):
        ...
        actual_versions = await get_versions_names_by_package(package['name'], 'MVN')
