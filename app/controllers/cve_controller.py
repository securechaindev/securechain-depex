...
from app.utils.ctc_parser import sanitize_version
    ...
    for raw_cpe_match in cpe_matches:
        ...
        if (
            ...
            if '*' in version:
                ...
                await update_versions_cves_by_constraints({'=': await sanitize_version(version)}, package_name, raw_cpe_match)
        else:
            ctcs = {}

            if 'versionStartIncluding' in cpe_match: ctcs['>='] = await sanitize_version(cpe_match['versionStartIncluding'])
            if 'versionEndIncluding' in cpe_match: ctcs['<='] = await sanitize_version(cpe_match['versionEndIncluding'])
            if 'versionStartExcluding' in cpe_match: ctcs['>'] = await sanitize_version(cpe_match['versionStartExcluding'])
            if 'versionEndExcluding' in cpe_match: ctcs['<'] = await sanitize_version(cpe_match['versionEndExcluding'])
