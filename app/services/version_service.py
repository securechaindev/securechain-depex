...
async def read_versions_by_constraints(constraints: dict[str, str], package_name: str) -> list:
...
async def read_versions_ids_by_constraints(constraints: dict[str, str], package_name: str) -> list:
    query = await get_complete_query(constraints, package_name)
...
async def update_versions_cves_by_constraints(constraints: list[list[str]], package_id: ObjectId, cve: dict) -> None:
    query = await get_complete_query(constraints, package_id)
    await version_collection.update_many(query, {'$addToSet': {'cves': cve}})

async def update_version_package_edges(version_id: ObjectId, package_edge_id: ObjectId) -> dict:
