...
async def update_env_variables_by_nvd(env_variables_id: str, now: datetime) -> None:
    env_variable_collection = get_collection('env_variables')
    await env_variable_collection.update_one({'_id': env_variables_id}, {'$set': {'nvd_last_update': now}})


async def update_env_variables_by_exploit_db(env_variables_id: str, now: datetime) -> None:
    env_variable_collection = get_collection('env_variables')
    await env_variable_collection.update_one({'_id': env_variables_id}, {'$set': {'exploit_db_last_update': now}})