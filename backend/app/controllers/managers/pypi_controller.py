...
async def pypi_create_package(
    ...
    for all_versions, name, constraints, parent_id, parent_version_name in api_versions_results:
        if all_versions:
            ...
            await pypi_extract_packages(api_requires_results)
...
async def pypi_extract_packages(
    ...
    for require_packages, version_id, name in api_requires_results:
        await pypi_generate_packages(require_packages, version_id, name)


async def pypi_search_new_versions(package: dict[str, Any]) -> None:
    api_versions_results = await get_pypi_versions(package["name"])
