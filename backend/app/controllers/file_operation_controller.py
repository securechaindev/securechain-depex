...
async def valid_file(
    ...
    if graph_data["name"] is not None:
        smt_transform = GraphToSMT(graph_data, valid_file_request.node_type, "mean")
...
async def minimize_impact(
    ...
    if graph_data["name"] is not None:
        smt_transform = GraphToSMT(graph_data, MinMaxImpactRequest.node_type, MinMaxImpactRequest.agregator)
        smt_text = await read_smt_text(smt_id)
        if smt_text is not None and smt_text["moment"].replace(tzinfo=UTC) > graph_data[
            "moment"
        ].replace(tzinfo=UTC):
        ...
        if not isinstance(result, str):
            result = await read_releases_by_counts(operation.get_result(), MinMaxImpactRequest.node_type)
        return JSONResponse(
...
async def filter_configs(
    ...
    if graph_data["name"] is not None:
        smt_transform = GraphToSMT(graph_data, FilterConfigsRequest.node_type, FilterConfigsRequest.agregator)
        smt_text = await read_smt_text(smt_id)
        if smt_text is not None and smt_text["moment"].replace(tzinfo=UTC) > graph_data[
            "moment"
        ].replace(tzinfo=UTC):
        ...
        result = operation.get_result()
        if not isinstance(result, str):
            result = await read_releases_by_counts(operation.get_result(), FilterConfigsRequest.node_type)
        return JSONResponse(
            status_code=status.HTTP_200_OK, content=json_encoder({"result": result, "message": "success"})
        )
    else:
